...
function newsBankFromStatus(status) {
  const stories = (status?.publishedBatch?.items || [])
  ...
  return status?.newsBank || { stories }
}
...
function buildDrafts(status) {
  if (status?.drafts?.drafts?.length) return status.drafts
...
export function buildDashboardSnapshot() {
  ...
  return {
    ...
    btsBlogInventory: normalizeBlogInventory(bts),
    ...
    btsDrafts: buildDrafts(bts),
    btsPublishLedger: bts?.publishLedger || buildPublishLedger(bts),
