...
function readFindingsLedger() {
  return readJson('bts/seo-findings-ledger.json', { findings: [] })
}

function readFaqBank() {
  return readJson('bts/faq-question-bank.json', { _meta: {}, faqs: [] })
}
...
function normalizeKeywords(status, keywordKey = 'locationKeywords') {
  ...
  return {
    ...
    // Pass through the BTS-produced ranked view (sorted position-ascending) so the
    // Rankings tab can render best→worst. Render-only consumer; not computed here.
    ranked: existing.ranked || null,
    rankedSummary: existing.rankedSummary || null,
    rankedGeneratedAt: existing.rankedGeneratedAt || null
  }
}
...
export function buildDashboardSnapshot() {
  ...
  const btsFindingsLedger = readFindingsLedger()
  const btsFaqBank = readFaqBank()
  ...
  return {
    ...
    dataSource: {
      ...
      files: ['bts-status.json', 'nbhw-status.json', 'dashboard-status.json', 'bts/content.json', 'bts/seo.json', 'bts/seo-findings-ledger.json', 'bts/faq-question-bank.json', 'bts/news-bank.json', 'bts/readiness.json'],
    ...
    btsFindings: btsFindingsLedger?.findings || [],
    btsFaqBank: btsFaqBank || { _meta: {}, faqs: [] },
    btsTraffic: buildTraffic(btsWithSeo, 'BTS-GA4-NOT-CONNECTED'),
    btsBing: btsWithSeo?.bingRankings || null,
    nbhwStatus: nbhw,
