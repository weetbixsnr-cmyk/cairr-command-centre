...
function buildDraftsFromContent(contentData, statusDrafts) {
  ...
  const drafts = items
    ...
    .map(i => {
      ...
      const hasFullText = !!(sd?.content && sd.content.length > 100)
      return {
        id: i.id,
        title: i.title,
        type: i.type,
        status: i.status,
        content: sd?.content || i.content || i.previewText || i.excerpt || '',
        hasFullText,
        placeholder: isPlaceholderContent(i, sd?.content || i.content || i.previewText || i.excerpt || ''),
        ...
        reviewGroup: sd?.reviewGroup || null,
