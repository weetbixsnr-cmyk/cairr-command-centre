...
function buildDraftsFromContent(contentData) {
  ...
  const drafts = items
    ...
    .map(i => ({
      ...
      status: i.status,
      content: i.excerpt || '',
      author: 'BTS content lifecycle',
      createdAt: i.publishedAt || null,
      batch: i.batch,
      sourceFile: i.sourceFile,
      reviewUrl: i.reviewUrl,
      service: i.service
    }))
  return { source: 'content.json', drafts }
}
...
function buildPublishLedgerFromContent(contentData) {
  ...
  return {
    ...
    last7d: {
      blogs: recent7.filter(i => ['blog', 'location'].includes(i.type)).length,
    ...
    last30d: {
      blogs: recent30.filter(i => ['blog', 'location'].includes(i.type)).length,
...
export function buildDashboardSnapshot() {
  ...
  return {
    ...
    dataSource: {
      ...
      files: ['bts-status.json', 'nbhw-status.json', 'dashboard-status.json', 'bts/content.json'],
    ...
    btsSeo: bts?.seo || null,
    btsKeywords: bts ? normalizeKeywords(bts, 'locationKeywords') : null,
    btsSeoplan: buildSeoPlan(bts, 'locationCoverage'),
    btsBlogInventory: useContentJson ? buildBlogInventoryFromContent(btsContent) : normalizeBlogInventory(bts),
    btsCompetitors: buildCompetitors(bts),
    btsCompetitorPages: buildCompetitorPages(bts),
    btsCourseDetails: buildCourseDetails(bts),
    btsCourses: buildCourses(bts),
    btsSuggestions: bts?.suggestions || { suggestions: [] },
    btsNotifications: bts?.notifications || { notifications: [] },
    btsDrafts: useContentJson ? buildDraftsFromContent(btsContent) : buildDrafts(bts),
    btsPublishLedger: useContentJson ? buildPublishLedgerFromContent(btsContent) : (bts?.publishLedger || buildPublishLedger(bts)),
    btsSeoDash: bts?.seoDash || buildSeoDash(bts),
    btsSeoAudit: bts?.seoAudit || buildSeoAudit(bts),
    btsTraffic: buildTraffic(bts, 'BTS-GA4-NOT-CONNECTED'),
    nbhwStatus: nbhw,
