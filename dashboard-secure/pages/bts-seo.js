      ...
      <div>
        ...
        {tab === 'future-posts' && (
          <>
            {(() => {
              const drafts = snap?.btsDrafts?.drafts || []
              const pending = drafts.filter(d => ['draft','sunny-editing'].includes(d.status))
              ...
              const doAction = async (id, action, extra = {}) => {
                ...
                try {
                  await fetch('/api/bts-draft-action', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
              ...
              const statusColor = {
                ...
                'approved': '#10b981',
                'visual-check-pending': '#a855f7',
                'signed-off': '#10b981'
              }
              const statusLabel = {
                'draft': '📝 Draft',
                'sunny-editing': '✏️ Sunny Editing',
                'approved': '✅ Approved',
                'visual-check-pending': '👁️ Visual Check',
                'signed-off': '🏁 Signed Off'
              }

              const DraftCard = ({ draft: d }) => {
                const isEditing = editingId === d.id
                const displayContent = d.editedContent || d.content
                return (
                  <div style={{background:'#111',border:'1px solid #222',borderRadius:10,padding:14,marginBottom:10,borderLeft:`3px solid ${statusColor[d.status] || '#333'}`}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                      <span style={{fontSize:8,padding:'2px 8px',borderRadius:4,fontWeight:600,background:`${statusColor[d.status]}20`,color:statusColor[d.status]}}>
                        {statusLabel[d.status] || d.status}
                      </span>
                      <span style={{fontSize:8,padding:'2px 8px',borderRadius:4,fontWeight:600,background:'#1a1a22',color:'#888',textTransform:'uppercase'}}>
                        {d.type}
                      </span>
                    ...
                    {isEditing ? (
                      <textarea
                        ...
                        style={{width:'100%',minHeight:200,padding:12,background:'#0a0a0a',border:'1px solid #3b82f6',borderRadius:8,color:'#fff',fontSize:12,fontFamily:'inherit',resize:'vertical',outline:'none',marginBottom:8}}
                      />
                    ) : (
                      <div style={{background:'#0a0a0d',border:'1px solid #1a1a22',borderRadius:8,padding:12,marginBottom:8,maxHeight:300,overflowY:'auto',fontSize:11,color:'#ccc',lineHeight:1.6,whiteSpace:'pre-wrap'}}>
                        {displayContent}
                      </div>
                    )}
                    ...
                    {/* Feedback */}
                    {d.feedback && (
                      <div style={{background:'#1a0a0a',border:'1px solid #ef444433',borderRadius:6,padding:8,marginBottom:8,fontSize:10,color:'#ef4444'}}>
                        💬 Feedback: {d.feedback}
                      </div>
                    )}
                    ...
                    {d.status === 'visual-check-pending' && (
                      <div style={{display:'flex',gap:12,marginBottom:8,padding:10,background:'#0a0a1a',borderRadius:6,border:'1px solid #a855f733'}}>
                        <label style={{display:'flex',alignItems:'center',gap:6,cursor:'pointer',fontSize:11,color:d.desktopChecked?'#10b981':'#888'}}>
                          <input type="checkbox" checked={d.desktopChecked} onChange={() => doAction(d.id, 'check-desktop')}
                            style={{width:16,height:16,cursor:'pointer'}} />
                          ✅ Checked on Desktop
                        </label>
                        <label style={{display:'flex',alignItems:'center',gap:6,cursor:'pointer',fontSize:11,color:d.mobileChecked?'#10b981':'#888'}}>
                          <input type="checkbox" checked={d.mobileChecked} onChange={() => doAction(d.id, 'check-mobile')}
                            style={{width:16,height:16,cursor:'pointer'}} />
                          ✅ Checked on Mobile
                        </label>
                      </div>
                    ...
                    <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                      {['draft','sunny-editing'].includes(d.status) && !isEditing && (
                        <button onClick={() => { setEditingId(d.id); setEditContent(displayContent) }}
                          style={{padding:'6px 14px',background:'#1a1a2a',border:'1px solid #3b82f6',borderRadius:6,color:'#3b82f6',fontSize:11,fontWeight:600,cursor:'pointer'}}>
                          ✏️ Edit
                        </button>
                      )}
                      {isEditing && (
                        <>
                          <button onClick={() => { doAction(d.id, 'edit', { content: editContent }); setEditingId(null) }}
                            disabled={actionLoading === d.id+'edit'}
                            style={{padding:'6px 14px',background:'#0a2a1a',border:'1px solid #10b981',borderRadius:6,color:'#10b981',fontSize:11,fontWeight:600,cursor:'pointer'}}>
                            💾 Save Changes
                          </button>
                          <button onClick={() => setEditingId(null)}
                            style={{padding:'6px 14px',background:'#1a1a1a',border:'1px solid #333',borderRadius:6,color:'#888',fontSize:11,fontWeight:600,cursor:'pointer'}}>
                            Cancel
                          </button>
                        </>
                      )}
                      {['draft','sunny-editing'].includes(d.status) && !isEditing && (
                        <button onClick={() => doAction(d.id, 'approve')}
                          disabled={actionLoading === d.id+'approve'}
                          style={{padding:'6px 14px',background:'#10b981',border:'none',borderRadius:6,color:'#fff',fontSize:11,fontWeight:700,cursor:'pointer'}}>
                          {actionLoading === d.id+'approve' ? '...' : '✅ Good to Go'}
                        </button>
                      )}
                    </div>

                    <div style={{fontSize:8,color:'#444',marginTop:6}}>
                      By {d.author} · Created {d.createdAt ? new Date(d.createdAt).toLocaleDateString('en-GB',{day:'numeric',month:'short'}) : '—'}
                      {d.editedBy && ` · Edited by ${d.editedBy}`}
                      {d.approvedAt && ` · Approved ${new Date(d.approvedAt).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}`}
                    </div>
                  </div>
                )
              }
              ...
              return (
                <>
                  ...
                  <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
                    {[
                      { label: 'Pending', count: pending.length, color: '#3b82f6' },
                      { label: 'Approved', count: approved.length, color: '#10b981' },
                      { label: 'Visual Check', count: visualCheck.length, color: '#a855f7' },
                      { label: 'Signed Off', count: signedOff.length, color: '#10b981' },
                    ].map((s, i) => (
                      <div key={i} style={{background:'#0d0d10',border:'1px solid #1a1a22',borderRadius:8,padding:'6px 14px',textAlign:'center',minWidth:80}}>
                        ...
                        <div style={{fontSize:8,color:'#888',textTransform:'uppercase',letterSpacing:0.5}}>{s.label}</div>
                      </div>
                    ))}
                  </div>

                  {/* Visual Check section — urgent */}
                  {visualCheck.length > 0 && (
                    <div style={{marginBottom:14}}>
                      <div className="sec-title" style={{color:'#a855f7'}}>👁️ Visual Check Required ({visualCheck.length})</div>
                      {visualCheck.map(d => <DraftCard key={d.id} draft={d} />)}
                    </div>
                  )}
                  ...
                  {pending.length > 0 && (
                    <div style={{marginBottom:14}}>
                      <div className="sec-title">📝 Drafts for Review ({pending.length})</div>
                      {pending.map(d => <DraftCard key={d.id} draft={d} />)}
                    </div>
                  )}

                  {/* Approved — waiting for publish */}
                  {approved.length > 0 && (
                    <div style={{marginBottom:14}}>
                      <div className="sec-title">✅ Approved — Publishing ({approved.length})</div>
                      {approved.map(d => <DraftCard key={d.id} draft={d} />)}
                    </div>
                  )}
                  ...
                  {signedOff.length > 0 && (
                    <div style={{marginBottom:14}}>
                      <div className="sec-title">🏁 Signed Off ({signedOff.length})</div>
                      {signedOff.slice(0, 5).map(d => <DraftCard key={d.id} draft={d} />)}
                      {signedOff.length > 5 && <div style={{fontSize:9,color:'#555',marginTop:4}}>+{signedOff.length - 5} more</div>}
                    </div>
                  )}
                  ...
                  {drafts.length === 0 && (
                    <div style={{textAlign:'center',padding:40,color:'#333'}}>
                      ...
                      <div style={{fontSize:13,color:'#555'}}>No future posts yet</div>
                      <div style={{fontSize:10,color:'#333',marginTop:4}}>BTS agent will submit drafts here for review</div>
                    </div>
                  )}
                </>
              )
