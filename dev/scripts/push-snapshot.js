...
function computePublishLedgerStats(filename) {
  ...
  const pagesLast7d = countByType(last7d, 'suburb') + countByType(last7d, 'location')
  const blogsLast7d = countByType(last7d, 'blog')
  const newsLast7d = countByType(last7d, 'news')
  const gbpLast7d = countByType(last7d, 'gbp')
  const pagesLast30d = countByType(last30d, 'suburb') + countByType(last30d, 'location')
  const blogsLast30d = countByType(last30d, 'blog')
  const newsLast30d = countByType(last30d, 'news')
  const gbpLast30d = countByType(last30d, 'gbp')

  const weeklyPageLimit = ledger.weeklyPageLimit || 3
  const weeklyBlogLimit = ledger.weeklyBlogLimit || 3
  const weeklyGbpLimit = ledger.weeklyGbpLimit || 3
  const weeklyNewsLimit = ledger.weeklyNewsLimit || 1

  // Status: red if ANY limit exceeded
  let status = 'green'
  const blogsOverLimit = blogsLast7d > weeklyBlogLimit
  const pagesOverLimit = pagesLast7d >= weeklyPageLimit
  const gbpOverLimit = gbpLast7d > weeklyGbpLimit
  const newsOverLimit = newsLast7d > weeklyNewsLimit
  if (blogsOverLimit || pagesOverLimit || gbpOverLimit || newsOverLimit) status = 'red'
  else if (pagesLast7d >= weeklyPageLimit - 1 || blogsLast7d >= weeklyBlogLimit) status = 'amber'

  // Build status label showing what's over
  let statusParts = []
  if (blogsOverLimit) statusParts.push(`blogs ${blogsLast7d}/${weeklyBlogLimit}`)
  if (pagesOverLimit) statusParts.push(`pages ${pagesLast7d}/${weeklyPageLimit}`)
  if (gbpOverLimit) statusParts.push(`GBP ${gbpLast7d}/${weeklyGbpLimit}`)
  if (newsOverLimit) statusParts.push(`news ${newsLast7d}/${weeklyNewsLimit}`)

  return {
    entries,
    totalPages: countByType(entries, 'suburb') + countByType(entries, 'location'),
    totalBlogs: countByType(entries, 'blog'),
    totalNews: countByType(entries, 'news'),
    totalGbp: countByType(entries, 'gbp'),
    last7d: { pages: pagesLast7d, blogs: blogsLast7d, news: newsLast7d, gbp: gbpLast7d, total: last7d.length },
    last30d: { pages: pagesLast30d, blogs: blogsLast30d, news: newsLast30d, gbp: gbpLast30d, total: last30d.length },
    weeklyPageLimit,
    weeklyBlogLimit,
    weeklyGbpLimit,
    weeklyNewsLimit,
    pagesRemaining: Math.max(0, weeklyPageLimit - pagesLast7d),
    blogsRemaining: Math.max(0, weeklyBlogLimit - blogsLast7d),
    gbpRemaining: Math.max(0, weeklyGbpLimit - gbpLast7d),
    newsRemaining: Math.max(0, weeklyNewsLimit - newsLast7d),
    status,
    statusLabel: status === 'green' ? `🟢 Safe to publish` :
                 status === 'amber' ? `🟡 Approaching limits — proceed with caution` :
                 `🔴 OVER LIMIT — ${statusParts.join(', ')} — hold all publishes`
  }
}
